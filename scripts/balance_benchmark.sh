...
if [ ! -s $OUTPUT_FILE ]; then
  echo "tree,tblsize,lsize,bsize,depth,fillratio,direction,time,writes,pmmwrites" >> $OUTPUT_FILE
fi
...
do
  ...
  do
    ...
    echo "${TREE}Tree,$elements,$lsize,$bsize,$depth,$fillratio,$direction,$time,$writes,$pmmwrites" >> $OUTPUT_FILE
  done
  popd > /dev/null
done
