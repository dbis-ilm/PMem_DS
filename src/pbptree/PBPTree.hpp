 ...
 *
 * PipeFabric is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY;
 * without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 * See the GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License along with PipeFabric.
  ...
  */
  ...
  void underflowAtLeafLevel(const pptr<BranchNode> &node, unsigned int pos,
    ...
    if (pos > 0 && prevNumKeys > middle) {
      ...
      nodeRef.keys.get_rw()[pos-1] = leafRef.keys.get_ro()[0];
      ...
      nodeRef.keys.get_rw()[pos] = leafRef.nextLeaf->keys.get_ro()[0];
    } else {
  ...
  pptr<LeafNode> mergeLeafNodes(const pptr<LeafNode> &node1, const pptr<LeafNode> &node2) {
    ...
    for (auto i = 0u; i < node2Ref.numKeys.get_ro(); i++) {
  ...
  void balanceLeafNodes(const pptr<LeafNode> &donor, const pptr<LeafNode> &receiver) {
    ...
    if (donorRef.keys.get_ro()[0] < receiverRef.keys.get_ro()[0]) {
      ...
      for (i = receiverRef.numKeys.get_ro(); i > 0; i--) {
      ...
      for (i = balancedNum; i < donorRef.numKeys.get_ro(); i++, j++) {
      ...
      /// move toMove keys/values from donor to receiver
      for (i = 0; i < toMove; i++) {
        receiverRef.keys.get_rw()[receiverRef.numKeys.get_ro()] = donorRef.keys.get_ro()[i];
        receiverRef.values.get_rw()[receiverRef.numKeys.get_ro()] = donorRef.values.get_ro()[i];
        receiverRef.numKeys.get_rw() = receiverRef.numKeys.get_ro() + 1;
      }
  ...
  void mergeBranchNodes(const pptr<BranchNode> &sibling, const KeyType &key,
    ...
    sibRef.keys.get_rw()[sibRef.numKeys.get_ro()] = key;
    sibRef.children.get_rw()[sibRef.numKeys.get_ro() + 1] = nodeRef.children.get_ro()[0];
    for (auto i = 0u; i < nodeRef.numKeys.get_ro(); i++) {
  ...
  pptr<BranchNode> underflowAtBranchLevel(const pptr<BranchNode> &node, unsigned int pos,
    ...
    if (pos > 0 &&
        (nodeRef.children.get_ro()[pos - 1]).branch->numKeys.get_ro() >
            middle) {
      /// we have a sibling at the left for rebalancing the keys
      pptr<BranchNode> sibling = (nodeRef.children.get_ro()[pos - 1]).branch;
      ...
      if (pos == 0) pos++;
      for (auto i = pos; i < nodeRef.numKeys.get_ro(); i++) {
  ...
  void balanceBranchNodes(const pptr<BranchNode> &donor, const pptr<BranchNode> &receiver,
    ...
    if (donorRef.keys.get_ro()[0] < receiverRef.keys.get_ro()[0]) {
      ...
      receiverRef.children.get_rw()[receiverRef.numKeys.get_ro() + toMove] =
          receiverRef.children.get_ro()[receiverRef.numKeys.get_ro()];
      for (i = receiverRef.numKeys.get_ro(); i > 0; i--) {
        /// reserve space on receiver side
        receiverRef.keys.get_rw()[i + toMove - 1] = receiverRef.keys.get_ro()[i - 1];
      ...
      /// mode from one node to a node with smaller keys
      unsigned int i = 0, n = receiverRef.numKeys.get_ro();
      ...
      donorRef.children.get_rw()[donorRef.numKeys.get_ro() - toMove] =
          donorRef.children.get_rw()[donorRef.numKeys.get_ro()];
  ...
  void printBranchNode(const unsigned int d, const pptr<BranchNode> &node) const {
    ...
    std::cout << d << " { ";
    for (auto k = 0u; k < nodeRef.numKeys.get_ro(); k++) {
      if (k > 0) std::cout << ", ";
  ...
  void printLeafNode(const unsigned int d, const pptr<LeafNode> &node) const {
    ...
    for (auto i = 0u; i < nodeRef.numKeys.get_ro(); i++) {
      if (i > 0) std::cout << ", ";
      std::cout << "{" << nodeRef.keys.get_ro()[i] /*<< " -> " << nodeRef.values.get_ro()[i]*/ << "}";
    }
    std::cout << "]" << std::endl;
  }
  ...
  void splitLeafNode(const pptr<LeafNode> &node, SplitInfo *splitInfo) {
    ...
    splitInfoRef.key = sibRef.keys.get_ro()[0];
  }
  ...
  void splitBranchNode(const pptr<BranchNode> &node, const KeyType &splitKey,
                       SplitInfo *splitInfo) {
    auto &nodeRef = *node;
    /// we have an overflow at the branch node, let's split it determine the split position
    ...
    /// move all entries behind this position to a new sibling node
    pptr<BranchNode> sibling = newBranchNode();
    auto &sibRef = *sibling;
    sibRef.numKeys.get_rw() = nodeRef.numKeys.get_ro() - middle;
    for (auto i = 0u; i < sibRef.numKeys.get_ro(); i++) {
    ...
    sibRef.children.get_rw()[sibRef.numKeys.get_ro()] = nodeRef.children.get_ro()[nodeRef.numKeys.get_ro()];
    ...
    if (hasSplit) {
      ...
      if (pos < hostRef.numKeys.get_ro()) {
        ...
        hostRef.children.get_rw()[hostRef.numKeys.get_ro() + 1] =
          hostRef.children.get_ro()[hostRef.numKeys.get_ro()];
  ...
  auto binarySearch(const pptr<BranchNode> &node, int l, int r, const KeyType &key) const {
    auto pos = 0u;
  ...
  auto binarySearch(const pptr<LeafNode> &node, int l, int r, const KeyType &key) const {
    auto pos = 0u;
    const auto &keys = node->keys.get_ro();
    while (l <= r) {
      pos = (l + r) / 2;
