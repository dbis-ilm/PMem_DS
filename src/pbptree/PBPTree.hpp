...
class PBPTree {
  ...
  */
  ...
  void insert(const KeyType &key, const ValueType &val) {
    ...
    transaction::run(pop, [&] {
        ...
        if (wasSplit) {
          ...
          root->numKeys.get_rw() = root->numKeys.get_ro() + 1;
  ...
  bool lookup(const KeyType &key, ValueType *val) const {
    ...
    if (pos < leafNode->numKeys.get_ro() && leafNode->keys.get_ro()[pos] == key) {
      // we found it!
  ...
  void balanceBranchNodes(persistent_ptr<BranchNode> donor, persistent_ptr<BranchNode> receiver,
    ...
    if (donor->keys.get_ro()[0] < receiver->keys.get_ro()[0]) {
      ...
      receiver->numKeys = receiver->numKeys + toMove;
    } else {
      // mode from one node to a node with smaller keys
    ...
    donor->numKeys = donor->numKeys - toMove;
  }
