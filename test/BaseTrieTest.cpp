...
TEST_CASE("Testing BaseTrie", "BaseTrie")
{
    const std::string path = dbis::gPmemPath + "BaseTrieTest";
    pmem::obj::pool<BaseTrieNode<int>> pop;
    if (access(path.c_str(), F_OK) != 0)
    {
        pop = pmem::obj::pool<BaseTrieNode<int>>::create(path, "BaseTrie", 1024 * 1024 * 16);
    }
    else
    {
        pop = pmem::obj::pool<BaseTrieNode<int>>::open(path, "BaseTrie");
    }
    ...
    SECTION("Lookup") {
        ...
        std::string ne("not existing");
        REQUIRE(!root->lookup(&ne, ne.size(), result));
    }
