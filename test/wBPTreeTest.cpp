...
TEST_CASE("Finding the leaf node containing a key", "[wBPTree]") {
  ...
  const auto alloc_class = pop.ctl_set<struct pobj_alloc_class_desc>("heap.alloc_class.128.desc",
                                                                     wBPTreeType4::AllocClass);

  if (!rootRef.btree10)
    transaction::run(pop, [&] { rootRef.btree10 = make_persistent<wBPTreeType10>(alloc_class); });

  if (!rootRef.btree4)
    transaction::run(pop, [&] { rootRef.btree4 = make_persistent<wBPTreeType4>(alloc_class); });

  if (!rootRef.btree3)
    transaction::run(pop, [&] { rootRef.btree3 = make_persistent<wBPTreeType6>(alloc_class); });

  if (!rootRef.btree12)
    transaction::run(pop, [&] { rootRef.btree12 = make_persistent<wBPTreeType12>(alloc_class); });

  if (!rootRef.btree20)
    transaction::run(pop, [&] { rootRef.btree20 = make_persistent<wBPTreeType20>(alloc_class); });
  ...
  SECTION("Balancing two inner nodes from left to right") {
    ...
    node1Ref.slot.get_rw()[1] = 0;
    node1Ref.slot.get_rw()[2] = 1;
    node1Ref.slot.get_rw()[0] = 2;
    ...
    node2Ref.slot.get_rw()[1] = 0;
    node2Ref.slot.get_rw()[2] = 1;
    node2Ref.slot.get_rw()[3] = 2;
    node2Ref.slot.get_rw()[4] = 3;
    ...
    node3Ref.slot.get_rw()[1] = 0;
    node3Ref.slot.get_rw()[0] = 1;
    ...
    REQUIRE(node1Ref.slot.get_ro()[0] == 2);
    REQUIRE(node2Ref.slot.get_ro()[0] == 2);
    REQUIRE(node3Ref.slot.get_ro()[0] == 3);
    ...
    for(auto i = 1u; i < node2Ref.slot.get_ro()[0] + 1; i++)
      actualKeys.push_back(node2Ref.keys.get_ro()[node2Ref.slot.get_ro()[i]]);
    REQUIRE(std::equal(std::begin(expectedKeys2), std::end(expectedKeys2),
            std::begin(actualKeys)));
    actualKeys.clear();
    for(auto i = 1u; i < node3Ref.slot.get_ro()[0] + 1; i++)
      actualKeys.push_back(node3Ref.keys.get_ro()[node3Ref.slot.get_ro()[i]]);
    REQUIRE(std::equal(std::begin(expectedKeys3), std::end(expectedKeys3),
            std::begin(actualKeys)));

    std::array<pptr<wBPTreeType4::LeafNode>, 3> node2Children;
    for (auto i = 1; i < 3; i++)
      node2Children[i-1] = node2Ref.children.get_ro()[node2Ref.slot.get_ro()[i]].leaf;
    ...
    for (auto i = 1; i < 4; i++)
      node3Children[i-1] = node3Ref.children.get_ro()[node3Ref.slot.get_ro()[i]].leaf;
  ...
  SECTION("Balancing two inner nodes from right to left") {
    ...
    node3Ref.slot.get_rw()[1] = 0;
    node3Ref.slot.get_rw()[2] = 1;
    node3Ref.slot.get_rw()[3] = 2;
    node3Ref.slot.get_rw()[4] = 3;
    node3Ref.slot.get_rw()[0] = 4;
    ...
    REQUIRE(node1Ref.slot.get_ro()[0] == 2);
    REQUIRE(node2Ref.slot.get_ro()[0] == 3);
    REQUIRE(node3Ref.slot.get_ro()[0] == 2);
  ...
  SECTION("Handling of underflow at a inner node by rebalance") {
    ...
    inner0Ref.bits.get_rw().set(0);
    inner0Ref.slot.get_rw()[1] = 0;
    inner0Ref.slot.get_rw()[0] = 1;
    ...
    inner1Ref.slot.get_rw()[1] = 0;
    inner1Ref.slot.get_rw()[0] = 1;
    ...
    inner2Ref.bits.get_rw().set(2);
    inner2Ref.slot.get_rw()[1] = 0;
    inner2Ref.slot.get_rw()[2] = 1;
    inner2Ref.slot.get_rw()[3] = 2;
    inner2Ref.slot.get_rw()[0] = 3;

    btree.underflowAtBranchLevel(innerNodes[0], 1, innerNodes[1]);

    REQUIRE(inner0Ref.slot.get_ro()[0] == 1);
    REQUIRE(inner0Ref.keys.get_ro()[0] == 9);

    REQUIRE(inner1Ref.slot.get_ro()[0] == 2);
    REQUIRE(inner2Ref.slot.get_ro()[0] == 2);
    ...
    std::vector<int> actualKeys{};
    for(auto i = 1u; i < inner2Ref.slot.get_ro()[0] + 1; i++)
      actualKeys.push_back(inner2Ref.keys.get_ro()[inner2Ref.slot.get_ro()[i]]);
  ...
  SECTION("Merging two inner nodes") {
    ...
    root1Ref.slot.get_rw()[1] = 0;
    root1Ref.slot.get_rw()[0] = 1;
    ...
    REQUIRE(node1Ref.slot.get_ro()[0] == 4);
  ...
  SECTION("Handling of underflow at a leaf node by merge and replace the root node") {
    ...
    parentRef.slot.get_rw()[1] = 0;
    parentRef.slot.get_rw()[0] = 1;
  ...
  SECTION("Handling of underflow at a leaf node by merge") {
    ...
    parentRef.slot.get_rw()[1] = 0;
    parentRef.slot.get_rw()[2] = 1;
    parentRef.slot.get_rw()[0] = 2;
    btree.rootNode = parent;

    btree.underflowAtLeafLevel(parent, 2, leaf2);
    REQUIRE(leaf1Ref.slot.get_ro()[0] == 5);
    REQUIRE(btree.rootNode.branch == parent);
    REQUIRE(parentRef.slot.get_ro()[0] == 1);
  }
  ...
  SECTION("Handling of underflow at a inner node") {
    ...
    inner1Ref.slot.get_rw()[1] = 0;
    inner1Ref.slot.get_rw()[2] = 1;
    inner1Ref.slot.get_rw()[0] = 2;
    ...
    REQUIRE(inner1Ref.slot.get_ro()[0] == 4);
    std::array<int, 4> expectedKeys{{5, 15, 20, 30}};
  ...
  SECTION("Inserting an entry into an inner node without a split") {
    ...
    nodeRef.slot.get_rw()[1] = 0;
    nodeRef.slot.get_rw()[0] = 1;

    btree.rootNode = node;
    btree.depth = 2;

    btree.insertInBranchNode(node, 1, 11, 112, &splitInfo);
    REQUIRE(leaf2->slot.get_ro()[0] == 3);
  }
  ...
  SECTION("Inserting an entry into an inner node with split") {
    ...
    REQUIRE(nodeRef.slot.get_ro()[0] == 2);

    std::array<int, 2> expectedKeys{{10, 12}};
    std::vector<int> actualKeys{};
    for(auto i = 1u; i < nodeRef.slot.get_ro()[0] + 1; i++)
      actualKeys.push_back(nodeRef.keys.get_ro()[nodeRef.slot.get_ro()[i]]);
    REQUIRE(std::equal(std::begin(expectedKeys), std::end(expectedKeys),
            std::begin(actualKeys)));
  ...
  SECTION("Constructing a B+ tree and iterating over it") {
    ...
    transaction::run(pop, [&] {
      ...
      btree = make_persistent<wBPTreeType10>(alloc_class);
      auto &btreeRef = *btree;
      for (int i = 0; i < 50; i++) {
